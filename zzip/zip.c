...
__zzip_fetch_disk_trailer(int fd, zzip_off_t filesize, struct _disk_trailer* _zzip_restrict trailer,
                          zzip_plugin_io_t io);
static int
__zzip_parse_root_directory(int fd, struct _disk_trailer* trailer, struct zzip_dir_hdr** hdr_return,
                            zzip_plugin_io_t io, zzip_off_t filesize);
...
#define __fixup_rootseek(_offset_of_trailer, _trailer) *(zzip_off_t*) _trailer = _offset_of_trailer;
                            ...
                            zzip_plugin_io_t io, zzip_off_t filesize)
{
    ...
    if (zz_entries <= 0 || zz_rootsize < 0 || zz_rootseek < 0 || zz_rootseek >= filesize)
        return ZZIP_CORRUPTED;
    ...
    if (USE_MMAP && io->fd.sys) {
        ...
        fd_map = _zzip_mmap(io->fd.sys, fd, zz_rootseek - zz_fd_gap, zz_rootsize + zz_fd_gap);
    ...
    for (entries = 0, zz_offset = 0;; entries++) {
        ...
        if (entries >= zz_entries) {
            if (zz_offset + 256 < zz_rootsize) {
                FAIL4("%li's entry is long before the end of directory - enable modulo_entries? "
                      "(O:%li R:%li)", /* .. */
        ...
        if ((zzip_off64_t) (zz_offset + sizeof(*d)) > zz_rootsize || /* .. */
            ...
            FAIL4("%li's entry stretches beyond root directory (O:%li R:%li)", (long) entries,
                  (long) (zz_offset), (long) zz_rootsize);
            break;
        }
...
static zzip_error_t
__zzip_dir_parse(ZZIP_DIR* dir); /* forward */
 ...
 */
ZZIP_DIR*
zzip_dir_fdopen_ext_io(int fd, zzip_error_t* errcode_p, zzip_strings_t* ext,
...
__zzip_dir_parse(ZZIP_DIR* dir)
{
    ...
    if ((rv = __zzip_fetch_disk_trailer(dir->fd, filesize, &trailer, dir->io)) != 0) {
        goto error;
    }

    HINT5("directory = { entries= %ld/%ld, size= %ld, seek= %ld } ",
          (long) _disk_trailer_localentries(&trailer), (long) _disk_trailer_finalentries(&trailer),
          (long) _disk_trailer_rootsize(&trailer), (long) _disk_trailer_rootseek(&trailer));

    if ((rv = __zzip_parse_root_directory(dir->fd, &trailer, &dir->hdr0, dir->io, filesize)) != 0) {
        goto error;
    }
error:
    return rv;
 ...
 */
ZZIP_DIR*
zzip_dir_open_ext_io(zzip_char_t* filename, zzip_error_t* e, zzip_strings_t* ext,
                     zzip_plugin_io_t io)
{
    int fd;
